...
const Environment = require("../uu5mock/environment.js");

const UU5StringObject = class UU5StringObject {
  /* constructor */
    ...
    */
    this.toChildren = (data, filterFn) => {
      ...
      if (filterFn) {
        ...
        result = { tag: resultTag, props: this.props.toChildren(data, (prop) => obj.props[prop.name], filterFn) };
        // return original props
        this.props.props = propsArray;
      } else {
      ...
      // generate value for props key
      if (!result.props) {
        result.props = {
          key: Tools.generateUUID()
        }
      } else if (!result.props.key) {
        if (!result.props.id) {
          result.props.key = Tools.generateUUID();
        } else {
          result.props.key = result.props.id;
        }
      }
